...
def get_home_reel_preview(viewer_id, limit=HOME_REEL_PREVIEW_LIMIT):
    ...
    try:
        ...
        viewed_ids = set()
        try:
            views_res = supabase.table('reel_views').select('reel_id').eq('user_id', viewer_id).execute()
...
def get_setup_health():
    ...
    if supabase:
        try:
            supabase.table('messages').select('attachment_url,deleted_for_everyone').limit(1).execute()
            checks.append(setup_health_check("Messages schema updates", True, "Attachments and deletion columns are available."))
        except Exception:
            checks.append(setup_health_check("Messages schema updates", False, "Run database/migrations/009_messages_attachments_deletion.sql in Supabase."))
    else:
        checks.append(setup_health_check("Messages schema updates", False, "Supabase is not configured."))
...
def send_message():
    ...
    if receiver_id and receiver_id != viewer['id'] and (content or temp_filename):
        if len(content) > 1000:
            ...
            flash(message, "error")
        elif content and recent_duplicate_submission('messages', {'sender_id': viewer['id'], 'receiver_id': receiver_id}, 'content', content):
            if request.form.get('ajax') == '1':
...
def delete_message():
    ...
    delete_type = request.form.get('delete_type', 'everyone') # 'everyone' or 'me'
    redirect_url = safe_redirect_url(request.form.get('redirect'), 'messages')
    wants_json = request.form.get('ajax') == '1'
    ...
    try:
        msg_res = supabase.table('messages').select('*').eq('id', message_id).execute()
        if not msg_res.data:
            if wants_json:
            ...
            if delete_type == 'everyone':
                ...
                try:
                    ...
                    if datetime.now(timezone.utc) - created_at > timedelta(minutes=15):
                        if wants_json:
                            return jsonify({'success': False, 'error': 'Delete for everyone time limit (15 minutes) exceeded.'}), 400
                        flash("Delete for everyone time limit (15 minutes) exceeded.", "error")
                        return redirect(redirect_url)
                except Exception:
                    pass
        ...
        if cooldown_until_str:
            try:
                ...
                if cooldown_until > now:
                    ...
                    cooldown_remaining = f"{diff.days} days, {diff.seconds // 3600} hours remaining"
            except Exception:
                pass
...
def log_admin_action(actor_username, action_type, target, details=""):
    try:
        ...
        log_line = f"[{timestamp}] admin={actor_username} action={action_type} target={target} details={details}\n"
        with open(log_file, 'a', encoding='utf-8') as f:
            f.write(log_line)
        app.logger.info(f"[ADMIN_AUDIT] {log_line.strip()}")
    except Exception as e:
        app.logger.error(f"[ADMIN_AUDIT_ERROR] Failed to write log: {e}")
...
def admin_dashboard():
        ...
        if action == 'login':
            ...
            if action == 'add_position':
                ...
                if title and department and description:
                    if supabase:
                        try:
                            supabase.table('job_positions').insert({
                                'title': title,
                            ...
                            log_admin_action(viewer['username'], 'add_position', title)
                        except Exception as exc:
                            flash(f"Error adding position: {exc}", "error")
                    return redirect(url_for('admin_dashboard'))
                ...
                if pos_id and supabase:
                    try:
                        ...
                        if res and res.data:
                            ...
                            supabase.table('job_positions').update({'is_active': new_status}).eq('id', pos_id).execute()
                            log_admin_action(viewer['username'], 'toggle_position', pos_id, f"new_status={new_status}")
                    except Exception as exc:
                        flash(f"Error toggling position: {exc}", "error")
                return redirect(url_for('admin_dashboard'))
                ...
                if pos_id and supabase:
                    try:
                        ...
                        log_admin_action(viewer['username'], 'delete_position', pos_id)
                    except Exception as exc:
                        flash(f"Error deleting position: {exc}", "error")
                return redirect(url_for('admin_dashboard'))
                ...
                if msg_id and status and supabase:
                    try:
                        ...
                        log_admin_action(viewer['username'], 'update_suggestion_status', msg_id, status)
                ...
                if req_id and status and supabase:
                    try:
                        req_res = supabase.table('verification_requests').select('*').eq('id', req_id).limit(1).execute()
                        if req_res and req_res.data:
                            req_data = req_res.data[0]
                            user_id = req_data['user_id']
                            updates = {'status': status, 'admin_notes': admin_notes, 'updated_at': datetime.now(timezone.utc).isoformat()}
                            if status == 'Rejected':
                                cooldown = datetime.now(timezone.utc) + timedelta(days=7)
                                updates['rejection_cooldown_until'] = cooldown.isoformat()
                                supabase.table('users').update({'is_profile_verified': False}).eq('id', user_id).execute()
                            elif status == 'Approved':
                                supabase.table('users').update({'is_profile_verified': True}).eq('id', user_id).execute()
                                updates['rejection_cooldown_until'] = None
                            supabase.table('verification_requests').update(updates).eq('id', req_id).execute()
                            log_admin_action(viewer['username'], 'respond_verification', req_id, f"status={status}")
                            flash("Verification request updated.", "success")
                    except Exception as exc:
                        flash(f"Error responding to verification: {exc}", "error")
                return redirect(url_for('admin_dashboard'))
                ...
                if user_id and warning_text and supabase:
                    try:
                        ...
                        # We send warning as a direct message from the admin user
                        supabase.table('messages').insert({
                            'sender_id': viewer['id'],
                            'receiver_id': user_id,
                        ...
                        flash(f"Error warning user: {exc}", "error")
                return redirect(url_for('admin_dashboard'))
                ...
                if user_id and supabase:
                    try:
                        ...
                        if res and res.data:
                            new_status = not res.data[0].get('is_profile_verified', False)
                            supabase.table('users').update({'is_profile_verified': new_status}).eq('id', user_id).execute()
                            log_admin_action(viewer['username'], 'toggle_user_verification', user_id, f"new_status={new_status}")
    ...
    if is_authenticated and supabase:
        try:
            res_p = supabase.table('job_positions').select('*').order('created_at', desc=True).execute()
            if res_p and hasattr(res_p, 'data') and res_p.data is not None:
                positions = res_p.data
        except Exception as exc:
            app.logger.warning(f"Failed to fetch job positions for admin: {exc}")
            
        try:
            res_a = supabase.table('job_applications').select('*').order('created_at', desc=True).execute()
            if res_a and hasattr(res_a, 'data') and res_a.data is not None:
                applications = res_a.data
        except Exception as exc:
            app.logger.warning(f"Failed to fetch job applications for admin: {exc}")
            
        try:
            res_m = supabase.table('contact_messages').select('*').order('created_at', desc=True).execute()
            if res_m and hasattr(res_m, 'data') and res_m.data is not None:
                all_msgs = res_m.data
                for msg in all_msgs:
                    if msg.get('subject') == 'Suggestion':
                        suggestions.append(msg)
                    else:
                        contact_messages.append(msg)
                messages = all_msgs
        except Exception as exc:
            app.logger.warning(f"Failed to fetch contact messages for admin: {exc}")

        try:
            res_v = supabase.table('verification_requests').select('*, user:users(*)').order('created_at', desc=True).execute()
            if res_v and hasattr(res_v, 'data') and res_v.data is not None:
                verification_requests = res_v.data
        except Exception as exc:
        ...
        try:
            ...
            app.logger.warning(f"Failed to fetch safety reports: {exc}")

        try:
            if q_user:
                res_u = supabase.table('users').select('*').or_(f"username.ilike.%{q_user}%,nickname.ilike.%{q_user}%,email.ilike.%{q_user}%,display_name.ilike.%{q_user}%").limit(50).execute()
            else:
                res_u = supabase.table('users').select('*').order('created_at', desc=True).limit(50).execute()
            if res_u and res_u.data:
                system_users = res_u.data
        except Exception as exc:
            app.logger.warning(f"Failed to fetch users: {exc}")
...
def download_attachment(message_id):
    ...
    try:
        ...
        # Check if viewer is one of the participants
        if viewer['id'] not in (msg.get('sender_id'), msg.get('receiver_id')):
            return abort(403)

        # Check if message is deleted
        if msg.get('sender_id') == viewer['id'] and msg.get('deleted_by_sender'):
            return abort(404)
        if msg.get('receiver_id') == viewer['id'] and msg.get('deleted_by_receiver'):
            return abort(404)
